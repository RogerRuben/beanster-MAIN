...
V18.1 对十个角色与四种头像按脸部锚点重新对齐，校正数据在 manifest 的 `alignment` 中。App 使用 `motion.js` 播放透明 PNG 帧，每次进入页面最多播放一次，点击后重播一次；全局只允许一个动画，结束后恢复静态 PNG。资源库的 GIF/WebP 仍保留循环格式用于外部集成。首页“鼠鼠表情”可查看全部 14 个表情并选择首页角色。
