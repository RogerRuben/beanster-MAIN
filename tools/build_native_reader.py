...
def build():
    ...
    subprocess.run([JAVA,'-m','jdk.compiler/com.sun.tools.javac.Main','--release','8','-encoding','UTF-8','-d',str(out),str(ROOT/'native/NativeReader.java')],check=True)
    jar=out/'reader.jar'
    with zipfile.ZipFile(jar,'w') as z:
        for p in sorted((out/'com').rglob('*.class')): z.write(p,p.relative_to(out).as_posix())
    subprocess.run([JAVA,'-cp',str(R8),'com.android.tools.r8.D8','--min-api','29','--output',str(out),str(jar)],check=True)
