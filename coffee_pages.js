...
const CoffeePages={
  ...
  eligible(){return ['today','collection'].includes(this.current())&&!AppNav.layers().length&&!CoffeeRoom.ritual},
  flip(direction=1){if(!this.eligible())return false;const target=this.current()==='today'?'collection':'today';Motion.stop();UI.go(target);requestAnimationFrame(()=>{if(this.current()===target&&!AppNav.layers().length)Motion.effect($(target),direction>0?'cc-enter-right':'cc-enter-left',220)});return true},
...
AppNav.back=function(){if(!AppNav.layers().length&&['today','collection'].includes(CoffeePages.current()))CoffeeRoom.finishRitual();if(CoffeePages.eligible())return CoffeePages.flip(1);return coffeePageBack()};
